...
check_vault_status() {
  ...
  if [[ $vault_status == *"Vault is sealed"* ]]; then
    echo "Vault is sealed. Waiting for unsealing..."
    return 1
  elif [[ $vault_status == *"connection refused"* ]]; then
    echo "Unable to connect to Vault. Waiting for Vault to start..."
    return 1
  else
    echo "Vault is unsealed and ready to receive requests."
    return 0
  fi
}
