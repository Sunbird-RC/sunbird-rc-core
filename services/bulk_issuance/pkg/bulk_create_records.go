...
func createRecords(params upload_and_create_records.PostV1EntityNameUploadParams, principal *models.JWTClaimBody) middleware.Responder {
	...
	totalSuccess, totalErrors, rows, err := services.ProcessDataFromCSV(&data, params.HTTPRequest.Header, params.EntityName)
	if err != nil {
	...
	id, err := services.InsertIntoFileData(rows, fileName, data, principal)
