...
export class Oid4vciService {
  ...
  async token(body: Record<string, any>) {
    ...
    // tx_code / user_pin check, against the value generated at offer creation
    // and distributed out of band by whoever created the offer.
    if (session.txCodeRequired) {
      ...
      if (!session.txCode) {
        // An offer that requires a PIN but stored none can only come from a
        // session written by an older build. Refuse rather than wave it through:
        // silently accepting any PIN is exactly the hole this replaced.
        this.logger.error(
          `Offer ${codeEntry.offerId} requires a tx_code but none was stored; refusing.`,
        );
        throw new BadRequestException('invalid_grant: offer has no transaction code');
      }
      if (!this.constantTimeEquals(String(pin), session.txCode)) {
        // The pre-auth code was already consumed above, so a wrong PIN burns the
        // offer instead of allowing unlimited guesses against a 6-digit secret.
        this.logger.warn(`Wrong tx_code for offer ${codeEntry.offerId}; offer consumed.`);
        throw new BadRequestException('invalid_grant: bad transaction code');
      }
    }
