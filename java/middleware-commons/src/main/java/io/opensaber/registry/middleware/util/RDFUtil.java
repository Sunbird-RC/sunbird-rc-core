...
public class RDFUtil {
		...
		while (stmtIterator.hasNext()) {
			...
			if (!node.getObject().isLiteral() && node.getObject().asResource().equals(object)) {
				...
				while (parentItr.hasNext()) {
					...
					logger.debug(
							"Updated corresponding parent statement(s) for RdfModel(s) : " + updatedParent.toString());
				}
			}
		}
	...
	/**
	 * This Method return updated model after adding signature statements if it not
	 * there else it will update existing signature value
	 * 
	 * @param model
	 ...
	 * @return Model
	 */
	public static Model getUpdatedSignedModel(Model model, String registryContext, String signatureDomain,
			Map entitySignMap, Model signatureModel) {
		...
		if (signatureModel.isEmpty()) {
			...
			model.add(r, ResourceFactory.createProperty(registryContext + Constants.SIGNATURE_FOR), "#",
					tm.getSafeTypeByName(XMLConstants.W3C_XML_SCHEMA_NS_URI + "#anyURI"));
		} else {
	...
	/**
	 * This Method filter Signature statements from the root model and returns root
	 * signature model separately
	 * 
	 * @param model
	 * @param registryContext
	 * @return model
	...
	public static Model removeAndRetrieveSignature(Model model, String registryContext) {
		...
		List<Resource> rootLabelList = getRootLabels(model);
		Resource target = rootLabelList.get(0);
		Literal literal = ResourceFactory.createTypedLiteral(target.toString(),
				tm.getSafeTypeByName(XMLConstants.W3C_XML_SCHEMA_NS_URI + "#anyURI"));
		ResIterator resIter = model.listSubjectsWithProperty(
				ResourceFactory.createProperty(registryContext + Constants.SIGNATURE_FOR), literal);
		if (resIter.hasNext()) {
			...
			Statement s = ResourceFactory.createStatement(target,
