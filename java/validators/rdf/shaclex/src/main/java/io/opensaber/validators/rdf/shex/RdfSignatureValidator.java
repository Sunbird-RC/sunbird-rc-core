...
public class RdfSignatureValidator {
	...
	// TODO: Instead of passing the ShapeType everytime, there could be a good
	// reason
	// to read all the shapes at once and then start validating against what was
	// read.
	public RdfSignatureValidator(Schema createSchema, String schemaContent, String registryContext,
			String registrySystemBase, String signatureConfigName, Map<String, String> shapeTypeMap) {
	...
	private Model getSignatureConfigModel() {
		return RDFUtil.getRdfModelBasedOnFormat(schemaForCreate.serialize(JSON_LD_FORMAT).right().get(),
				JSON_LD_FORMAT);
	}
	...
	private void addAttributesForShape(RDFNode firstNode, Model signatureConfigModel, List<String> attributeList) {
		// Getting object nodes by filtering based on different predicates iteratively
		// till all attributes for the shape are filtered
		...
		if (!secondNode.equals(RDF.nil)) {
			...
			RDFNode fourthNode = RDFUtil.getFirstObject((Resource) thirdNode, SHAPE_PREDICATE_IRI,
	...
	private void validateSignatureFields(Model rdfModel) throws MiddlewareHaltException {
		...
		while (rdfIter.hasNext()) {
			...
			if (!rNode.isAnon() && !predicate.equals(RDF.type) && !signatureAttributes.contains(predicateStr)
				...
				// Filtering statements based on the signatureFor attribute to check with
				// signature exists for each attribute
				ResIterator subjectIter = rdfModel.listSubjectsWithProperty(prop,
						ResourceFactory.createTypedLiteral(predicateStr, rdt));
				...
				NodeIterator nodeIter = rdfModel.listObjectsOfProperty(s.getSubject(),
						ResourceFactory.createProperty(registryContext + Constants.SIGNATURES));
				...
				while (nodeIter.hasNext() && !entitySignatureFound) {
					...
					entitySignatureFound = rdfModel.contains((Resource) node, prop,
							ResourceFactory.createTypedLiteral(rNodeStr, rdt));
				}
