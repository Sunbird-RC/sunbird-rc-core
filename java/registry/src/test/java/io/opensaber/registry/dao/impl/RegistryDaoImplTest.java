...
public class RegistryDaoImplTest extends RegistryTestBase {
	...
	@Test
	public void test_adding_a_single_node() throws DuplicateRecordException, RecordNotFoundException,
			EncryptionException, LabelCannotBeNullException, AuditFailedException {
	...
	private int checkIfAuditRecordsAreRight(Graph entity, Map<String, Map<String, Integer>> updateCountMap)
			throws LabelCannotBeNullException {
		boolean auditEnabled = environment.getProperty("audit.enabled") != null
				? Boolean.parseBoolean(environment.getProperty("audit.enabled"))
				: false;
		int count = 0;
		if (auditEnabled) {
			int adjustedCount;
			...
			while (it.hasNext()) {
				...
				count += adjustedCount;
				assertEquals(adjustedCount,
						auditRecordReader.fetchAuditRecords(String.valueOf(pair.getKey()), null).size());
				it.remove();
			}
		}
		return count;
	...
	public void test_adding_multiple_nodes() throws NullPointerException, DuplicateRecordException,
			RecordNotFoundException, EncryptionException, AuditFailedException, LabelCannotBeNullException {
		Model rdfModel = getNewValidRdf();
		String rootLabel = updateGraphFromRdf(rdfModel);
		String response = registryDao.addEntity(graph, String.format("_:%s", rootLabel), null, null);
		Graph entity = registryDao.getEntityById(response, false);
		...
		// assertEquals(getPropCounterMap(entity), new
		// AuditRecordReader(databaseProvider).fetchAuditRecords(response,null).size());
	}
	...
	public void test_adding_shared_nodes() throws LabelCannotBeNullException {
		try {
			...
			long verticesCountAfterSharedNodesCreation = IteratorUtils
					.count(databaseProvider.getGraphStore().traversal().clone().V()
							.filter(v -> !v.get().label().equalsIgnoreCase(Constants.GRAPH_GLOBAL_CONFIG))
							.hasNot(Constants.AUDIT_KEYWORD));
			long edgesCountAfterSharedNodesCreation = IteratorUtils
					.count(databaseProvider.getGraphStore().traversal().clone().E().hasNot(Constants.AUDIT_KEYWORD));
	...
	@Test
	public void test_adding_shared_nodes_with_new_properties() throws DuplicateRecordException, RecordNotFoundException,
			EncryptionException, AuditFailedException, LabelCannotBeNullException {
		...
		String addressLabel = databaseProvider.getGraphStore().traversal().clone().V()
				.has(T.label, "http://example.com/voc/teacher/1.0.0/IndianUrbanPostalAddress").next()
		...
		String propertyValue = (String) entity.traversal().clone().V()
				.properties("http://example.com/voc/teacher/1.0.0/districtAlias").next().value();
	...
	public void test_adding_shared_nodes_with_updated_properties() throws DuplicateRecordException,
		...
		Model newRdfModel = getNewValidRdf();
		updateNodeLabel(newRdfModel, "http://example.com/voc/teacher/1.0.0/IndianUrbanPostalAddress");
		removeStatementFromModel(newRdfModel,
				ResourceFactory.createProperty("http://example.com/voc/teacher/1.0.0/municipality"));
		...
		String newEntityResponse = registryDao.addEntity(newEntityGraph, String.format("_:%s", newRootLabel), null,
				null);
		Graph entity = registryDao.getEntityById(newEntityResponse, false);
		...
		String propertyValue = (String) entity.traversal().clone().V()
				.properties("http://example.com/voc/teacher/1.0.0/municipality").next().value();
	...
	public void test_count_nested_node_with_first_node_as_blank_node()
			throws NullPointerException, DuplicateRecordException, RecordNotFoundException, NoSuchElementException,
			EncryptionException, AuditFailedException {
	...
	public void test_blank_node_count_when_no_blank_node_present()
		...
		GraphTraversal<Vertex, Vertex> blankNodes = entity.traversal().clone().V()
				.filter(v -> v.get().label().startsWith("_:")).V();
	...
	public void test_update_when_entity_not_exists() throws DuplicateRecordException, RecordNotFoundException,
			EncryptionException, NoSuchElementException, AuditFailedException {
	...
	@Test
	public void test_update_single_literal_node() throws DuplicateRecordException, RecordNotFoundException,
		...
		/*
		 * removeStatementFromModel(updateRdfModel, ResourceFactory.createProperty(
		 * "http://example.com/voc/teacher/1.0.0/clusterResourceCentre"));
		 * removeStatementFromModel(updateRdfModel,
		 * ResourceFactory.createProperty("http://example.com/voc/teacher/1.0.0/address"
		 * )); updateNodeLabel(updateRdfModel,
		 * "http://example.com/voc/teacher/1.0.0/School");
		 */
		...
		/*
		 * Model updateRdfModelWithoutType = getModelwithOnlyUpdateFacts(rdfModel,
		 * updateRdfModel,Arrays.asList());
		...
		updatedGraphResult.traversal().V().properties("http://example.com/voc/teacher/1.0.0/revenueBlock")
	...
	private Model getModelwithOnlyUpdateFacts(Model rdfModel, Model updateRdfModel) {
		...
		/*
		 * Property propertyToRemove; propertyToRemove=ResourceFactory.createProperty(
		 * "http://www.w3.org/1999/02/22-rdf-syntax-ns#type"); Model
		 * updateRdfModelWithoutType = restrictModel(updateRdfModel,propertyToRemove);
		 * Iterator<String> iter = predicatedToExclude.iterator();
		 * while(iter.hasNext()){ propertyToRemove=
		 * ResourceFactory.createProperty(iter.next()); updateRdfModelWithoutType =
		 * restrictModel(updateRdfModelWithoutType,propertyToRemove); }
		 * System.out.println("ORIGINAL FACTS are"); printModel(rdfModel);
	...
	private Map<String, Map<String, Integer>> generateUpdateMapFromRDF(Model updateRdfModelWithoutType,
		...
		while (iter.hasNext()) {
			...
			if (!encounteredSubPred.containsKey(subject) || (encounteredSubPred.containsKey(subject)
				...
				logger.debug(String.format("Number of facts deleted for %s and %s : %d", subject.toString(),
						predicate.toString(), deletedCount));
			}
	...
	public void test_update_multiple_literal_nodes() throws DuplicateRecordException, RecordNotFoundException,
		...
		updatedGraphResult.traversal().V()
				.properties(String.format("%s/revenueBlock", prefix), String.format("%s/clusterResourceCentre", prefix))
	...
	public void test_add_iri_node_to_existing_entity()
		...
		String newResponse = registryDao.addEntity(updateGraph, label, response,
				"http://example.com/voc/teacher/1.0.0/address");
		...
		/*
		 * Model updateRdfModelWithoutType = getModelwithOnlyUpdateFacts(rdfModel,
		 * updateRdfModel,Arrays.asList("http://example.com/voc/teacher/1.0.0/address"))
		 * ; checkIfAuditRecordsAreRight(updatedGraphResult,generateUpdateMapFromRDF(
		 * updateRdfModelWithoutType));
		 */
		Model addedModel = JenaRDF4J.asJenaModel(RDF2Graph.convertGraph2RDFModel(entity, response));
		...
		updatedGraphResult
				.traversal().clone().V().properties(String.format("%s/mohalla", prefix),
						String.format("%s/municipality", prefix), String.format("%s/city", prefix))
				.forEachRemaining(p -> result.add(p.value().toString()));
	...
	public void test_update_iri_node_and_literal_nodes()
		...
		Model addedModel = JenaRDF4J.asJenaModel(RDF2Graph.convertGraph2RDFModel(entity, response));
		Model updateRdfModel = createRdfFromFile("update_node.jsonld", response);
		StmtIterator stmt = addedModel.listStatements(ResourceFactory.createResource(response),
				ResourceFactory.createProperty("http://example.com/voc/teacher/1.0.0/address"), (RDFNode) null);
		while (stmt.hasNext()) {
			Statement s = stmt.next();
			Resource subject = s.getSubject();
			Property predicate = s.getPredicate();
			if (subject.toString().equals(response)
					&& predicate.toString().equalsIgnoreCase("http://example.com/voc/teacher/1.0.0/address")) {
				RDFNode rdfNode = s.getObject();
				updateRdfModel.add(s);
				updateRdfModel.add((Resource) rdfNode,
						ResourceFactory.createProperty("http://example.com/voc/teacher/1.0.0/mohalla"),
						"Updated Sector 14");
				updateRdfModel.add((Resource) rdfNode,
						ResourceFactory.createProperty("http://example.com/voc/teacher/1.0.0/municipality"),
						"Updated MCG");
		...
		updatedGraphResult.traversal().V()
				.properties(String.format("%s/revenueBlock", prefix), String.format("%s/clusterResourceCentre", prefix),
						String.format("%s/mohalla", prefix), String.format("%s/municipality", prefix))
				.forEachRemaining(p -> result.add(p.value().toString()));

		List<String> expected = Arrays.asList("updated block", "Updated Cluster Resource", "Updated Sector 14",
				"Updated MCG");
		assertEquals(5, IteratorUtils.count(updatedGraphResult.traversal().clone().V()));
		assertThat(result, is(expected));
	...
	@Test
	public void savingMetaProperties()
			throws DuplicateRecordException, RecordNotFoundException, EncryptionException, AuditFailedException {
	...
	/*
	 * @Test public void test_delete_non_existing_root_node() throws Exception {
	 * expectedEx.expect(RecordNotFoundException.class);
	 * expectedEx.expectMessage(Constants.ENTITY_NOT_FOUND); String id =
	 * generateRandomId(); Model rdfModel = ModelFactory.createDefaultModel();
	 * rdfModel.add(ResourceFactory.createResource(id), null, (RDFNode)null); String
	 * rootLabel = updateGraphFromRdf(rdfModel);
	 * registryDao.deleteEntity(graph,rootLabel); }
	 * 
	 * @Test public void test_delete_non_existing_nested_node() throws Exception {
	 * expectedEx.expect(RecordNotFoundException.class);
	 * expectedEx.expectMessage(Constants.ENTITY_NOT_FOUND); Model rdfModel =
	 * getNewValidRdf(); String rootLabel = updateGraphFromRdf(rdfModel); String
	 * response = registryDao.addEntity(graph, String.format("_:%s", rootLabel));
	 * //Graph entity = registryDao.getEntityById(response); String id =
	 * generateRandomId(); Model rdfDeleteModel = ModelFactory.createDefaultModel();
	 * rdfDeleteModel.add(ResourceFactory.createResource(response), null,
	 * (RDFNode)ResourceFactory.createResource(id)); rootLabel =
	 * updateGraphFromRdf(rdfDeleteModel);
	 * //registryDao.deleteEntity(graph,response); }
	 * 
	 * @Test public void test_delete_existing_node() throws Exception { Model
	 * rdfModel = getNewValidRdf(); String rootLabel = updateGraphFromRdf(rdfModel);
	 * String response = registryDao.addEntity(graph, String.format("_:%s",
	 * rootLabel)); Model rdfDeleteModel = ModelFactory.createDefaultModel();
	 ...
	 * (RDFNode)ResourceFactory.createResource(
	 * "http://example.com/voc/teacher/1.0.0/AreaTypeCode-URBAN")); rootLabel =
	 * updateGraphFromRdf(rdfDeleteModel);
	 * //registryDao.deleteEntity(graph,response); Graph updatedGraphResult =
	 * registryDao.getEntityById(response);
	 * assertFalse(updatedGraphResult.traversal().E().hasLabel(
	 * "http://example.com/voc/teacher/1.0.0/area").hasNext()); }
	 */
	@Test
	public void test_update_properties() throws Exception {
		...
		Model updateRdfModel = createRdfFromFile("update_node.jsonld", response);
		removeStatementFromModel(updateRdfModel,
				ResourceFactory.createProperty("http://example.com/voc/teacher/1.0.0/clusterResourceCentre"));
		/*
		 * removeStatementFromModel(updateRdfModel,
		 * ResourceFactory.createProperty("http://example.com/voc/teacher/1.0.0/address"
		 * )); updateNodeLabel(updateRdfModel,
	...
	public void test_update_single_valued_properties() throws Exception {
		...
		removeStatementFromModel(updateRdfModel,
				ResourceFactory.createProperty("http://example.com/voc/teacher/1.0.0/clusterResourceCentre"));
		removeStatementFromModel(updateRdfModel,
				ResourceFactory.createProperty("http://example.com/voc/teacher/1.0.0/revenueBlock"));
		...
		assertEquals("http://example.com/voc/teacher/1.0.0/AreaTypeCode-RURAL", updatedGraphResult.traversal().E()
				.hasLabel("http://example.com/voc/teacher/1.0.0/area").next().inVertex().label());
	}
	...
	@Test
	public void test_for_delete_root_node() throws DuplicateRecordException, NoSuchElementException,
			EncryptionException, AuditFailedException, RecordNotFoundException {
	...
	@Test
	public void test_for_delete_child_node_with_root_status_active() throws DuplicateRecordException,
			NoSuchElementException, EncryptionException, AuditFailedException, RecordNotFoundException {
		/*
		 * expectedEx.expect(UnsupportedOperationException.class);
	...
	@Test
	public void test_for_delete_root_node_child_node() throws DuplicateRecordException, NoSuchElementException,
			EncryptionException, AuditFailedException, RecordNotFoundException {
		/*
		 * expectedEx.expect(UnsupportedOperationException.class);
		 * expectedEx.expectMessage(Constants.DELETE_UNSUPPORTED_OPERATION_ON_ENTITY);
	...
	@Test
	public void test_for_delete_when_entity_not_exists()
			throws RecordNotFoundException, NoSuchElementException, AuditFailedException {
