...
public class CustomJwtAuthenticationConverterTest {
    ...
    public void shouldExtractValuesFromJWTClaim() {
        ...
        assertEquals("1234", ((UserToken) authenticationToken).getUserId());
        assertEquals("{name=1}", ((UserToken) authenticationToken).getConsentFields().toString());
        assertEquals("1234@mail.com", ((UserToken) authenticationToken).getEmail());
        assertEquals("[Student, Teacher]", ((UserToken) authenticationToken).getEntities().toString());
        assertEquals("[admin]", authenticationToken.getAuthorities().toString());
    }
    ...
    public void shouldHandleInvalidValuesWhileExtractionWithDefaultValues() {
        ...
        assertNotNull(authenticationToken);
        assertEquals("1234", ((UserToken) authenticationToken).getUserId());
        assertEquals("{}", ((UserToken) authenticationToken).getConsentFields().toString());
        assertEquals("1234@mail.com", ((UserToken) authenticationToken).getEmail());
        assertEquals("[]", ((UserToken) authenticationToken).getEntities().toString());
        assertEquals("[]", authenticationToken.getAuthorities().toString());
    }
}